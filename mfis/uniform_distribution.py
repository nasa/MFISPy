...
class IndependentUniformDistribution(InputDistribution):
    ...
    def evaluate_pdf(self, samples):
        p = self._get_uniform_density(samples.shape[0])
        
        indices_outside_bounds_samples = \
                self._indices_of_samples_outside_bounds(samples)
        if len(indices_outside_bounds_samples) > 0:
    ...
    def _get_uniform_density(self, num_of_samples):
        uniform_density = 1
        for i,bound in enumerate(self._bounds):
            uniform_density *= 1/(bound[1] - bound[0])
        p = np.ones((num_of_samples,1)) * uniform_density
        
        return p  

       
    def _indices_of_samples_outside_bounds(self, samples):
